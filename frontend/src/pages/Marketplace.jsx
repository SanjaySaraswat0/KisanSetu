  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-gray-900">🌾 KisanSetu Direct Produce Marketplace</h1>
          <p className="text-gray-600 text-sm">Direct market linkage between verified farmers, FPOs, and bulk buyers.</p>
        </div>
        <div className="flex gap-2">
          {["All", "Wheat", "Onion"].map((crop) => (
            <button
              key={crop}
              onClick={() => setFilterCrop(crop)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${
                filterCrop === crop ? "bg-emerald-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
              }`}
            >
              {crop}
            </button>
